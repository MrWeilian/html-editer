...
function Rfix(){
	...
	/*var script = '<script class="fixScript" >$(function(){var oLf'+Dindex+' = $(".block").eq('+Dindex+').offset().left/parseInt($("html").css("font-size"));function setFix'+Dindex+'(){var wW = $(window).width();var bW = $("body").width(); var nowL = ((wW-bW)/2)/parseInt($("html").css("font-size")) + oLf'+Dindex+';$(".block").eq('+Dindex+').css("left",""+ nowL +"rem");}setFix'+Dindex+'();window.addEventListener("resize",function(){setFix'+Dindex+'();});});</script>';*/
    ...
    if(Ctop >= wH){//判断top大于(可视区高-元素高)直接设置为bottom0
    	...
    	$this.css({"top":"0rem","position":"fixed"});
    }else{
        $this.css({"top":""+ Ctop / 20 +"rem","position":"fixed"});
    }
